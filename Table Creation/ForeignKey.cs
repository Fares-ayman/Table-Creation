    ...
    public partial class ForeignKey : UserControl
    {
        ...
        private void read_tables()
        {
            if (File.Exists("Tables.xml"))
            {
                ...
                {
                    if (Form1.tableName != list[i].Attributes["name"].Value)
                    {
        ...
        private void AddColBtn_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            XmlNodeList list_tables = doc.SelectNodes("//Table[@name='" + Form1.tableName + "']");
            ...
            this.Parent.Controls.Add(table);
            table.BringToFront();
